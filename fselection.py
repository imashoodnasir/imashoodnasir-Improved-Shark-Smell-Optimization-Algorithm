...
def fselection(X_train, Y):
    ...
    for i in range(gen):
        ...
        IrreleventFeatures = R_22[np.newaxis]
        
        
        Darwin_Guy = FinalFeatures[:]
        Not_So_Darwin_Guy = IrreleventFeatures[:]
    ...
    FFeature = np.vstack((Min_for_all_Generations_for_Mut_1_1_1,
    ...
    for i in FFeature:
        if(FFeature[t,1]) <= min(FFeature[:,1]):
            FinalFeatures = FFeature[t,:]
        t = t+1
    
    FFeatures_Final = FinalFeatures[np.newaxis]
    
    print()
    print("Min in all Generations:",FFeatures_Final)
    
    print()
    print("Final Solution:",FFeatures_Final[:,2:])
    print("Highest Accuracy:",(1-FFeatures_Final[:,1]))
    
    
    
    A11 = FFeatures_Final[:,2:][0]
